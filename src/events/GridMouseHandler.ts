    ...
    public handleMouseMove(e: MouseEvent, handler: InteractionHandler): void {
        ...
        if (handler["resizeState"]) {
            if (handler["resizeState"].type === "column") {        
                ...
                if (col) col.width = Math.max(30, handler["resizeState"].startSize + deltaX);
            } else {
                const deltaY = e.clientY - handler["resizeState"].startPos;
                const row = this.workbook.rows[handler["resizeState"].index];
                if (row) row.height = Math.max(15, handler["resizeState"].startSize + deltaY);
            }
            this.updateView();
            return;
        }
        ...
        if (handler["isSelectingRange"] && handler.selection) {
            ...
            if (indices) {
                if (handler["dragSelectionType"] === "column" && indices.colIdx !== -1) {
                    ...
                    handler.selection.type = (handler.selection.startColIdx === handler.selection.endColIdx) ? "column" : "range";
                } 
                    ...
                    handler.selection.type = (handler.selection.startRowIdx === handler.selection.endRowIdx) ? "row" : "range";
                } 
